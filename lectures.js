...
async function readFile(req, res) {
  let data = '';
  try {
    data = await readFileAsync('lectures.json');
  } catch (e) {
...
async function displayLectures(req, res) {
  lectureData = await readFile(req, res);
  res.render('lectures', { lectures: lectureData.lectures });
}
...
async function lecture(req, res, next) {
  ...
  let lectures = await readFile(req, res);
  lectures = lectures.lectures
  const lecture = lectures.find(a => a.slug === slug);

  if (!lecture) {
    return next();
  }
  const contents = lecture.content;

  let items = createContent(contents);

  res.render('lecture', { items: items.innerHTML });
}

router.get('/', catchErrors(displayLectures));
